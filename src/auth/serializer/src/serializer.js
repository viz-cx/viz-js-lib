...
class Serializer {
    ...
    try {
      ...
      for (let i = 0; i < iterable.length; i++) {
        ...
        try {
          ...
          ErrorWithCause.throw;
        }
      }

    } catch (error) {
      ErrorWithCause.throw(`${this.operation_name}.${field}`, error);
    }
  ...
  appendByteBuffer(b, object) {
    ...
    try {
      ...
      try {
        ErrorWithCause.throw(`${this.operation_name}.${field} = ${ JSON.stringify(object[field])}`, error);
      } catch { // circular ref
        ErrorWithCause.throw(`${this.operation_name}.${field} = ${ object[field]}`, error);
      }
    }
    return;
  }
  ...
  toObject(serialized_object = {}, debug = { use_default: false, annotate: false }){
    ...
    try {
      ...
      for (let i = 0; i < iterable.length; i++) {
        field = iterable[i];
        const type = this.types[field];
        const object = type.toObject(((typeof serialized_object !== 'undefined' && serialized_object !== null) ? serialized_object[field] : undefined), debug);
        result[field] = object;
        if(HEX_DUMP) {
  ...
  fromHex(hex) {
    const b = ByteBuffer._fromHex(hex, ByteBuffer.LITTLE_ENDIAN);
